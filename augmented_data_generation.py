...
cfg = load_config("Wonder3D/configs/mvdiffusion-joint-ortho-6views.yaml")
...
orj_images_path = glob.glob("Wonder3D/horse_combined/train/*/*rgb.jpg") # jpg for horse, png for bird dataset.
os.makedirs("Wonder3D/horse_augmented_dataset_fixed", exist_ok=True)

count = 0
for orj_image_path in orj_images_path[0:]: # if training interrupt, change this count with last folder name + 1
    ...
    orj_bbox_path = orj_image_path.replace("rgb.jpg", "box.txt")
    ...
    # new dataset path
    new_dataset_path = "Wonder3D/horse_augmented_dataset_fixed/train"
    ...
    for i, view in enumerate(views):
        ...
        x_min, x_max = x, x + w
        y_min, y_max = y, y + h
        # crop the view with bbox and a little padding
        x_min = max(0, x - (x_max - x_min) // 5) # 5 is magic number so if you want change it!
        y_min = max(0, y - (y_max - y_min) // 5)
        x_max = min(view.size[0], x + w + (x_max - x_min) // 5)
        y_max = min(view.size[1], y + h + (y_max - y_min) // 5)
        view = view.crop((x_min, y_min, x_max, y_max))
        view = view.resize((256, 256), Image.LANCZOS)
